...
var WYSIWYG = {
	...
	stripURLPath: function(n, content, exact) {
		...
		if(this.config[n].ImagePathToStrip == "auto") {
			stripImageUrl = WYSIWYG_Core.getDocumentUrl(document);
		}
		else if(this.config[n].ImagePathToStrip != "") {
			stripImageUrl = this.config[n].ImagePathToStrip;
		}
		...
		if(stripImageUrl) {
			...
			url = WYSIWYG_Core.stringToRegex(WYSIWYG_Core.getDocumentPathOfUrl(stripImageUrl));	
			...
			result = WYSIWYG_Core.getDocumentPathOfUrl(stripImageUrl).match(/.+[\/]{2,3}[^\/]*/,"");
			if(result) {
