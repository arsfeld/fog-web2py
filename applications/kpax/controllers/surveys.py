...
def index():
    surveys_owned=accessible('survey',('take',))\
         ((db.survey.stop>=today)|(db.survey.owner==user_id))\
         (db.user.id==db.survey.owner)\
         .select(orderby=~db.survey.stop|db.survey.id)
    surveys_todo=accessible('survey',('take',))\
         ((db.survey.stop>=today)|(db.survey.owner==user_id))\
         (db.user.id==db.survey.owner)\
         (~db.survey.id.belongs(db(db.sa.owner==user_id)\
...
def create_survey():
    ...
    if form.accepts(request.vars,session):
        db.access.insert(table_name='survey',record_id=form.vars.id,\
                         users_group=g_tuple[1],access_type="take")
        db.sa.insert(survey=form.vars.id,owner=user_id)
        session.flash='suryvey created'       
        redirect(URL(r=request,f='edit_questions',args=[form.vars.id]))
...
def edit_survey():
    ...
    if form.accepts(request.vars,session):
        ...
        redirect(URL(r=request,f='edit_questions',args=request.args))
    return dict(form=form)
...
def edit_questions():
    ...
    rows=db(db.question.survey==survey.id)\
           ...
           (db.survey.owner==user_id).select(orderby=db.question.number)
    ...
    if question_id>0:
        qrows=db(db.question.id==question_id)\
                ...
                (db.survey.owner==user_id).select()
        if len(qrows):
            form=SQLFORM(db.question,qrows[0].question,showid=False,
                 fields=question_fields,deletable=db.survey.owner==user_id)
            if form.accepts(request.vars,session):
...
def append_question():
    ...
    args=[survey.id,i]
    redirect(URL(r=request,f='edit_questions',args=args))
...
def take_survey():
    ...
    if question.type=='short text':
        ...
        upload=URL(r=request,f='download_attachment',args=[sa.id])
...
def report():
    ...
    if len(request.args)<2:
       sas=db(db.sa.survey==survey.id)\
             (db.sa.owner==db.user.id).select(orderby=db.user.name)
       rows=db(db.question.survey==survey.id)\
              (db.answer.question==db.question.id)\
              (db.answer.sa==db.sa.id)\
              (db.user.id==db.sa.owner).select(orderby=db.question.id|db.user.name)
    else:
       ...
       rows=db(db.question.survey==survey.id)\
              (db.answer.question==db.question.id)\
              (db.answer.sa==db.sa.id)\
              (db.user.id==db.sa.owner)\
              (db.user.id==request.args[1]).select(orderby=db.question.id|db.user.name)
    total_points={}
    points={}
    for r in sas: 
        total_points[r.user.id]=0.0
        points[r.user.id]={}
        for row in rows:
            if row.user.id==r.user.id:
               points[r.user.id][row.question.id]=p=0.0
               ...
               points[r.user.id][row.question.id]=p
               total_points[r.user.id]+=p
...
def delete_sa():
    if len(request.args)<2: redirect(URL(r=request,f='index'))
    survey=mysurvey()
    if int(request.args[1])!=user_id:
        db(db.sa.owner==request.args[1])(db.sa.survey==survey.id).delete()
    redirect(URL(r=request,f='report',args=[survey.id]))
...
def upload_survey():
    ...
    try:   
        ...
        db.access.insert(table_name='survey',record_id=i,\
                         users_group=g_tuple[1],access_type="take")
        db.sa.insert(survey=i,owner=user_id)
    except:
    ...
    try:
        for item in data.questions_response:
            ...
            redirect(URL(r=request,f='edit_survey',args=[i]))
    except:
