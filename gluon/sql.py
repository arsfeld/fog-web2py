...
SQL_DIALECTS = {
    ...
    'mssql': {
        ...
        'reference FK': ', CONSTRAINT FK_%(table_name)s_%(field_name)s__constraint FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s',
    ...
    'ingres': {
        ...
        #'reference FK': ', CONSTRAINT FK_%(table_name)s_%(field_name)s__constraint FOREIGN KEY (%(field_name)s) REFERENCES %(foreign_key)s ON DELETE %(on_delete_action)s', ## FIXME TODO
...
class SQLDB(dict):
    ...
    def define_table(
        ...
        **args
        ...
        for key in args:
            if key not in ['migrate','primarykey']:
    ...
    def import_from_csv_file(self, ifile, id_map={}, null='<NULL>', unique='uuid'):
        for line in ifile:
            line = line.strip()
            if not line:
                continue
...
''' % (t, t, t))
    ...
    # KeyedTable
    def insert(self, **fields):
        if self._db._dbname in ['mssql', 'mssql2', 'db2', 'ingres']:
            query = self._insert(**fields)
            self._db['_lastsql'] = query
            try:
                self._db._execute(query)
...
def update_record(colset, table, id, a={}):
    b = a or dict(colset)
    c = dict([(k,v) for (k,v) in b.items() if k in table.fields and not k=='id'])
...
class Rows(object):
    ...
    def __getslice__(self, a, b):
        return Rows(seld.db,self.records[a:b],self.colnames)
...
def DAL(uri='sqlite:memory:', pool_size=0, folder=None):
    if uri == 'gae':
