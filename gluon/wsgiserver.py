...
##                'SSL_SERVER_V_END': Validity of server's certificate (end time),
    ...
    def makefile(self, sock, mode='r', bufsize=-1):
        if SSL and isinstance(sock, SSL.ConnectionType):
            ...
            return CP_fileobject(sock, mode, bufsize)
...
class CherryPyWSGIServer(object):
    ...
    def start(self):
        ...
        if not self.ssl_adapter and hasattr(self,'ssl_certificate') \
                and hasattr(self,'ssl_private_key'):
            try:
                self.ssl_adapter = pyOpenSSLAdapter(self.ssl_certificate,
                                                    self.ssl_private_key)
            except:
                logging.error('Malfunction in pyOpenSSLAdapter. OpenSSL missing?')
...
if __name__=='__main__':
    server = CherryPyWSGIServer(('127.0.0.1', int(sys.argv[1])), my_crazy_app, request_queue_size=1024)
    server.start()
